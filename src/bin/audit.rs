...
impl AuditRecord {
    pub fn commitment(&self) -> String {
        let data = format!("{}:{}:{}:{}:{}",
            self.operation, self.timestamp, self.input_commitment,
            self.output_commitments.join("|"), self.prev_audit);
    ...
    pub fn to_shard(&self) -> Shard {
        ...
        let pairs = vec![
            ...
            ("output_count".into(), self.output_commitments.len().to_string()),
            ("output_commitments".into(), self.output_commitments.join(",")),
...
fn latest_audit(dir: &str) -> String {
    let mut audits: Vec<PathBuf> = fs::read_dir(dir)
        ...
        .map(|e| e.path())
        .filter(|p| p.file_name().map(|n| n.to_string_lossy().starts_with("audit_")).unwrap_or(false))
        .collect();
    audits.sort();
    audits.last()
...
fn cmd_log(operation: &str, input_path: &str, output_dir: &str) -> Result<()> {
    ...
    eprintln!("audit: {} → {} ({} outputs, commitment {})",
        operation, path, output_commitments.len(), &commit[..16]);
    Ok(())
}
...
fn cmd_verify(dir: &str) -> Result<()> {
    ...
    for entry in fs::read_dir(dir)? {
        let path = entry?.path();
        if path.file_name().map(|n| n.to_string_lossy().starts_with("audit_")).unwrap_or(false) {
    ...
    for (_, raw) in &audits {
        ...
        let debug = format!("{:?}", ciborium::from_reader::<ciborium::Value, _>(data).ok());
    ...
    let real_violations = if violations > audits.len() { violations - audits.len() } else { 0 };

    if real_violations == 0 {
        eprintln!("✓ COMPLIANT: {} shards, {} audits, root={}", hashes.len(), audits.len(), &root[..16]);
    } else {
        eprintln!("✗ VIOLATION: {} unaccounted shards out of {}", real_violations, hashes.len());
    }
...
fn cmd_chain(dir: &str) -> Result<()> {
    ...
    for path in &audits {
        ...
        eprintln!("{} → {}", path.file_name().unwrap().to_string_lossy(), &hash[..16]);
    }
    Ok(())
}
